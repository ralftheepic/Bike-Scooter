...
const Sales = () => {
  ...
  // Loading States (Existing - for Buy and Sell)
  ...
  // Removed loadingDashboard state - Dashboard component manages its own


  // Error States (Existing - for Buy and Sell)
  const [errorBulkOrders, setErrorBulkOrders] = useState(null);
  const [errorIndividualBills, setErrorIndividualBills] = useState(null);
   // Removed errorDashboard state - Dashboard component manages its own


  // Filter States for "Buy" (Bulk Orders) (Existing)
  const [filterBuyDistributor, setFilterBuyDistributor] = useState('');
  const [filterBuyDate, setFilterBuyDate] = useState('');

  // Filter States for "Sell" (Individual Bills) (Existing)
  ...
  // Removed Dashboard Aggregated Data states


  // --- Data Fetching ---

  // Fetch Bulk Orders (Existing - Reverted to original useEffect dependency)
  useEffect(() => {
    const fetchBulkOrders = async () => {
      ...
      try {
        ...
        setBulkOrders(data);
        console.log("Fetched Bulk Orders:", data);
      } catch (error) {
...
    // Removed the useEffect for fetching dashboard data - Dashboard component handles this
  ...
  const filteredIndividualBills = individualBills.filter(bill => {
      ...
      const matchesPriceLessThan = filterSellPriceLessThan === '' || isNaN(filterSellLT) || billTotalAmount <= filterLT;
...
                            <td className="border px-4 py-2 text-right">₹{item.product?.price?.toFixed(2) || '0.00'}</td>
                        </tr>
                        ))}
