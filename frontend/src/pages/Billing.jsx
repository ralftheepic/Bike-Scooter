...
          const searchString = `${product.name || ''} ${product.brand || ''} ${product.model || ''} ${product.partNumber || ''} ${product.productId || ''} ${product.description || ''}`;
...
                  // Keep existing customer data if response is unexpected
                  // setCustomerName(customerName); // Retain current state
                  // setCustomerPhoneNumber(customerPhoneNumber); // Retain current state
              }
          } else {
...
                // Keep existing customer data if response is incomplete
                // setCustomerName(customerName); // Retain current state
                // setCustomerPhoneNumber(customerPhoneNumber); // Retain current state
                return;
            }
...
            // --- Update state with canonical customer data from the finalized bill ---
            setCustomerName(finalizedBill.customerName || ''); // Use canonical name from backend
            setCustomerPhoneNumber(finalizedBill.customerPhoneNumber || ''); // Use canonical phone from backend
            // --------------------------------------------------------------------
...
            setPaymentRefId(null);
            // Keep existing customer data on finalize failure
            // setCustomerName(customerName); // Retain current state
            // setCustomerPhoneNumber(customerPhoneNumber); // Retain current state
          }
        } catch (error) {
...
                          >
                            {`${product.name || ''} ${product.brand ? `(${product.brand})` : ''} ${product.model ? `[${product.model}]` : ''} ${product.partNumber ? `(PN: ${product.partNumber})` : ''} ${product.description ? `- ${product.description}` : ''}`.trim()}
